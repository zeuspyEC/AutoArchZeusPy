...
show_menu() {
    ...
    echo -e "${COLORS[HEADER]}${'═':%${#title}s}${COLORS[RESET]}\n"
    
    for i in "${!options[@]}"; do
        echo -e "${COLORS[PRIMARY]}$((i+1)). ${COLORS[INFO]}${options[$i]}${COLORS[RESET]}"
    done
    
    echo -e "\n${COLORS[INPUT]}Seleccione una opción (1-${#options[@]}): ${COLORS[RESET]}"
}
...
show_progress() {
    ...
    printf "\r${COLORS[INFO]}[%-${width}s] %d%%" "$(printf '%*s' "$filled" | tr ' ' '█')" "$percent"
...
check_dependencies() {
    ...
    echo -e "\n${COLORS[HEADER]}Verificando dependencias requeridas:${COLORS[RESET]}\n"
    
    for dep in "${deps[@]}"; do
        echo -ne "${COLORS[INFO]}Verificando $dep... ${COLORS[RESET]}"
        if command -v "$dep" >/dev/null 2>&1; then
            echo -e "${COLORS[SUCCESS]}✔${COLORS[RESET]}"
        else
            echo -e "${COLORS[ERROR]}✘${COLORS[RESET]}"
            missing_deps+=("$dep")
        fi
    done
...
check_system_requirements() {
    ...
    local arch
    arch=$(uname -m)
    echo -ne "\n${COLORS[INFO]}Verificando arquitectura... ${COLORS[RESET]}"
    if [[ "$arch" != "x86_64" ]]; then
        echo -e "${COLORS[ERROR]}✘${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Verificando modo de arranque... ${COLORS[RESET]}"
    if [[ -d "/sys/firmware/efi/efivars" ]]; then
        BOOT_MODE="UEFI"
        echo -e "${COLORS[SUCCESS]}UEFI${COLORS[RESET]}"
    else
        BOOT_MODE="BIOS"
        echo -e "${COLORS[SUCCESS]}BIOS${COLORS[RESET]}"
    fi
    ...
    echo -ne "${COLORS[INFO]}Verificando memoria RAM... ${COLORS[RESET]}"
    if ((total_ram < min_ram)); then
        echo -e "${COLORS[ERROR]}✘${COLORS[RESET]}"
        log "ERROR" "RAM insuficiente: ${total_ram}MB < ${min_ram}MB"
        return 1
    fi
    echo -e "${COLORS[SUCCESS]}✔ ${total_ram}MB${COLORS[RESET]}"
...
verify_disk_space() {
    ...
    echo -e "\n${COLORS[HEADER]}Discos disponibles:${COLORS[RESET]}\n"
    ...
    while IFS= read -r disk_info; do
        ...
        if ((disk_size >= min_space)); then
            echo -e "${COLORS[SUCCESS]}$disk_count. $disk_name - $(numfmt --to=iec-i --suffix=B "$disk_size") - $disk_model${COLORS[RESET]}"
            valid_disks+=("$disk_name")
        else
            echo -e "${COLORS[ERROR]}$disk_count. $disk_name - $(numfmt --to=iec-i --suffix=B "$disk_size") - $disk_model (Espacio insuficiente)${COLORS[RESET]}"
        fi
    done <<< "$disk_list"
...
check_network_connectivity() {
    ...
    echo -e "\n${COLORS[HEADER]}Probando conectividad:${COLORS[RESET]}\n"
    
    for host in "${test_hosts[@]}"; do
        echo -ne "${COLORS[INFO]}Probando conexión a $host... ${COLORS[RESET]}"
...
setup_wifi_connection() {
    ...
    echo -e "\n${COLORS[HEADER]}Interfaces Wi-Fi disponibles:${COLORS[RESET]}\n"
    for i in "${!wireless_interfaces[@]}"; do
        echo -e "${COLORS[PRIMARY]}$((i+1)). ${wireless_interfaces[$i]}${COLORS[RESET]}"
    done
    
    echo -e "\n${COLORS[INPUT]}Seleccione una interfaz (1-${#wireless_interfaces[@]}): ${COLORS[RESET]}"
    ...
    # Mostrar redes
    echo -e "\n${COLORS[HEADER]}Redes disponibles:${COLORS[RESET]}\n"
    iwctl station "$selected_interface" get-networks
    
    # Solicitar datos de conexión
    echo -e "\n${COLORS[INPUT]}Ingrese el nombre de la red (SSID): ${COLORS[RESET]}"
    read -r ssid
    
    echo -e "${COLORS[INPUT]}Ingrese la contraseña: ${COLORS[RESET]}"
    read -rs password
    
    echo -e "\n${COLORS[INFO]}Conectando a $ssid...${COLORS[RESET]}"
...
setup_ethernet_connection() {
    ...
    echo -e "\n${COLORS[HEADER]}Interfaces Ethernet disponibles:${COLORS[RESET]}\n"
    
    for interface in "${ethernet_interfaces[@]}"; do
        interface=$(echo "$interface" | tr -d ' ')
        echo -ne "${COLORS[INFO]}Configurando $interface... ${COLORS[RESET]}"
...
select_installation_disk() {
    ...
    echo -e "\n${COLORS[HEADER]}Discos disponibles para instalación:${COLORS[RESET]}\n"
    
    local disk_array=()
    while IFS= read -r disk_info; do
        disk_array+=("$disk_info")
        local number=$((${#disk_array[@]}))
        echo -e "${COLORS[PRIMARY]}$number. ${COLORS[INFO]}$disk_info${COLORS[RESET]}"
    done <<< "$disk_list"
    
    while true; do
        echo -e "\n${COLORS[INPUT]}Seleccione el disco para la instalación (1-${#disk_array[@]}): ${COLORS[RESET]}"
        read -r selection
        ...
        echo -e "${COLORS[ERROR]}Selección inválida${COLORS[RESET]}"
    done
...
check_existing_os() {
    echo -e "\n${COLORS[HEADER]}Verificando sistemas operativos existentes:${COLORS[RESET]}\n"
    ...
    if command -v os-prober &>/dev/null; then
        ...
        if [[ -n "$existing_os" ]]; then
            echo -e "${COLORS[WARNING]}¡Se encontraron los siguientes sistemas operativos!${COLORS[RESET]}"
            echo -e "$existing_os" | while IFS= read -r os; do
                echo -e "${COLORS[WARNING]}► $os${COLORS[RESET]}"
            done
        else
            echo -e "${COLORS[INFO]}No se encontraron otros sistemas operativos${COLORS[RESET]}"
        fi
    fi
}

show_warning_message() {
    echo -e "\n${COLORS[ERROR]}¡ADVERTENCIA!${COLORS[RESET]}"
    echo -e "${COLORS[ERROR]}Esta operación eliminará TODOS los datos en $TARGET_DISK${COLORS[RESET]}"
    echo -e "${COLORS[ERROR]}Esta acción NO se puede deshacer${COLORS[RESET]}\n"
    
    while true; do
        echo -e "${COLORS[INPUT]}¿Está seguro que desea continuar? (si/NO): ${COLORS[RESET]}"
        read -r response
        
        case "$response" in
            [Ss][Ii]) break ;;
            [Nn][Oo]|"") log "INFO" "Operación cancelada por el usuario"; exit 1 ;;
            *) echo -e "${COLORS[ERROR]}Por favor responda 'si' o 'no'${COLORS[RESET]}" ;;
        esac
    done
}
...
create_uefi_partitions() {
    ...
    echo -e "\n${COLORS[HEADER]}Esquema de particionamiento:${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición EFI: ${efi_size}MB${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición ROOT: ${root_size}MB${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición SWAP: ${swap_size}MB${COLORS[RESET]}\n"
    ...
    echo -ne "${COLORS[INFO]}Creando particiones...${COLORS[RESET]}"
    ...
    echo -e "\n${COLORS[HEADER]}Formateando particiones:${COLORS[RESET]}"
    
    echo -ne "${COLORS[INFO]}Formateando partición EFI... ${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Formateando partición ROOT... ${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Formateando partición SWAP... ${COLORS[RESET]}"
    ...
    # Montar particiones
    echo -e "\n${COLORS[HEADER]}Montando particiones:${COLORS[RESET]}"
    
    echo -ne "${COLORS[INFO]}Montando partición ROOT... ${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Creando y montando directorio EFI... ${COLORS[RESET]}"
...
create_bios_partitions() {
    ...
    echo -e "\n${COLORS[HEADER]}Esquema de particionamiento:${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición BOOT: ${boot_size}MB${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición ROOT: ${root_size}MB${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Partición SWAP: ${swap_size}MB${COLORS[RESET]}\n"
    ...
    echo -ne "${COLORS[INFO]}Formateando partición BOOT... ${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Creando y montando directorio BOOT... ${COLORS[RESET]}"
...
verify_partitions() {
    ...
    echo -e "\n${COLORS[HEADER]}Particiones creadas:${COLORS[RESET]}\n"
...
install_base_system() {
    ...
    echo -e "\n${COLORS[HEADER]}Instalando paquetes base:${COLORS[RESET]}\n"
    local total_packages=${#REQUIRED_PACKAGES[@]}
    local current=0
    
    for package in "${REQUIRED_PACKAGES[@]}"; do
        ((current++))
        echo -ne "${COLORS[INFO]}Instalando $package... ${COLORS[RESET]}"
    ...
    # Generar fstab
    echo -ne "\n${COLORS[INFO]}Generando fstab... ${COLORS[RESET]}"
...
update_mirrors() {
    ...
    echo -ne "${COLORS[INFO]}Respaldando mirrorlist actual... ${COLORS[RESET]}"
    ...
    if command -v reflector &>/dev/null; then
        echo -ne "${COLORS[INFO]}Optimizando mirrors con reflector... ${COLORS[RESET]}"
...
configure_system() {
    ...
    for func in "${config_functions[@]}"; do
        ((current++))
        echo -e "\n${COLORS[HEADER]}[${current}/${total_steps}] Ejecutando: ${func//_/ }${COLORS[RESET]}"
        if ! $func; then
...
configure_hostname() {
    echo -e "${COLORS[INPUT]}Ingrese el hostname para el sistema: ${COLORS[RESET]}"
    while true; do
        read -r HOSTNAME
        ...
        if [[ "$HOSTNAME" =~ ^[a-zA-Z0-9-]+$ ]]; then
            ...
            echo -e "${COLORS[ERROR]}Hostname inválido. Use solo letras, números y guiones${COLORS[RESET]}"
            echo -e "${COLORS[INPUT]}Intente nuevamente: ${COLORS[RESET]}"
        fi
    done
...
configure_timezone() {
    ...
    echo -e "\n${COLORS[HEADER]}Zonas horarias disponibles:${COLORS[RESET]}\n"
    ...
    echo -ne "${COLORS[INFO]}Configurando zona horaria $TIMEZONE... ${COLORS[RESET]}"
...
configure_locale() {
    echo -ne "${COLORS[INFO]}Configurando locales... ${COLORS[RESET]}"
...
configure_users() {
    # Configurar root
    echo -e "\n${COLORS[HEADER]}Configuración de contraseña root${COLORS[RESET]}"
    while ! arch-chroot /mnt passwd; do
        echo -e "${COLORS[ERROR]}Error al configurar contraseña root. Intente nuevamente${COLORS[RESET]}"
    done
    
    # Crear usuario normal
    echo -e "\n${COLORS[HEADER]}Creación de usuario normal${COLORS[RESET]}"
    echo -e "${COLORS[INPUT]}Ingrese nombre para el nuevo usuario: ${COLORS[RESET]}"
    while true; do
        read -r USERNAME
        ...
        if [[ "$USERNAME" =~ ^[a-z][a-z0-9-]*$ ]]; then
            ...
            echo -e "${COLORS[ERROR]}Nombre de usuario inválido. Use letras minúsculas, números y guiones${COLORS[RESET]}"
            echo -e "${COLORS[INPUT]}Intente nuevamente: ${COLORS[RESET]}"
        fi
    done
    ...
    echo -e "\n${COLORS[HEADER]}Configuración de contraseña para $USERNAME${COLORS[RESET]}"
    while ! arch-chroot /mnt passwd "$USERNAME"; do
        echo -e "${COLORS[ERROR]}Error al configurar contraseña. Intente nuevamente${COLORS[RESET]}"
    done
...
configure_network() {
    echo -ne "${COLORS[INFO]}Habilitando NetworkManager... ${COLORS[RESET]}"
...
configure_bootloader() {
    ...
    if [[ "$BOOT_MODE" == "UEFI" ]]; then
        echo -ne "${COLORS[INFO]}Instalando GRUB para UEFI... ${COLORS[RESET]}"
        ...
        echo -ne "${COLORS[INFO]}Instalando GRUB para BIOS... ${COLORS[RESET]}"
    ...
    echo -ne "${COLORS[INFO]}Generando configuración de GRUB... ${COLORS[RESET]}"
...
cleanup() {
    ...
    echo -ne "${COLORS[INFO]}Desmontando particiones... ${COLORS[RESET]}"
...
main() {
    ...
    for step in "${installation_steps[@]}"; do
        ((current++))
        echo -e "\n${COLORS[HEADER]}[${current}/${total_steps}] ${step//_/ }${COLORS[RESET]}"
        if ! $step; then
    ...
    echo -e "\n${COLORS[HEADER]}Resumen de la Instalación${COLORS[RESET]}"
    echo -e "${COLORS[SUCCESS]}✔ Instalación completada exitosamente${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Tiempo total: ${minutes}m ${seconds}s${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Hostname: $HOSTNAME${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Usuario: $USERNAME${COLORS[RESET]}"
    echo -e "${COLORS[INFO]}• Zona horaria: $TIMEZONE${COLORS[RESET]}"
    
    # Preguntar por reinicio
    echo -e "\n${COLORS[INPUT]}¿Desea reiniciar el sistema ahora? (si/NO): ${COLORS[RESET]}"
    ...
    if [[ "$reboot_choice" =~ ^[Ss][Ii]$ ]]; then
        ...
        cleanup
        echo -e "${COLORS[WARNING]}Recuerde reiniciar el sistema cuando esté listo${COLORS[RESET]}"
    fi
}

# Iniciar instalación solo si se ejecuta como root
if [[ $EUID -ne 0 ]]; then
    echo -e "${COLORS[ERROR]}Este script debe ejecutarse como root${COLORS[RESET]}"
    exit 1
fi
